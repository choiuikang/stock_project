...
for i in range(5):    
 ...
 #stock.get_market_ohlcv_by_date 주식데이터 시작일,종료일,티커 파라미터를 넣어주면 일자별로 정렬하여 DataFrame으로 반환한다.
    ...
    if not os.path.exists('ranging_stock_df_python3.csv'):
        stock_df.to_csv('ranging_stock_df_python3.csv', index=False, mode='w')
    else:
        stock_df.to_csv('ranging_stock_df_python3.csv', index=False, mode='a', header=False)
